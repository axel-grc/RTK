...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Checks that an image has a satisfactory MSE against a reference.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--reference", "-i", required=True, type=rtk.comma_separated_args(str), help="reference image(s)"
    )
    parser.add_argument(
        "--reconstruction", "-j", required=True, type=rtk.comma_separated_args(str), help="reconstruction image(s)"
    )
    parser.add_argument(
        "--threshold", "-t", required=True, type=rtk.comma_separated_args(float), help="MSE threshold(s)"
    )
    parser.add_argument(
...
def process(args_info: argparse.Namespace):
    ...
    n_max = max(len(args_info.reference), len(args_info.reconstruction), len(args_info.threshold))
