...
def process(args_info: argparse.Namespace):
    ...
    if args_info.input:
        ...
        # Set 4th dimension (frames): prefer 4-component --size if provided, else --frames
        size = constantImageSource.GetSize()
        if (
            hasattr(args_info, "size")
            and isinstance(args_info.size, list)
            and len(args_info.size) >= 4
        ):
            size[3] = int(args_info.size[3])
        else:
